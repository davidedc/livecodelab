...
of the mesh is. There are more possible types such as particles, etc. but they
are not currently used in LiveCodeLab.
...
been deprecated, see https://github.com/mrdoob/three.js/issues/751 and instead a
createMultiMaterialObject utility was put in place, which basically
...
"Spinning"
----------------------
"Spinning" applies to all objects added to an empty frame: it makes all objects
spin for a few frames. This has been implemented for two reasons a) cosmetic b)
the user is likely to first use "box", and without spinning that would look like
a boring square that appears without animation. Spinning gives many more cues:
